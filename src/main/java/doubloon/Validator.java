...
public class Validator implements Producer {
    ...
    public void produce(String message) {
        ...
        try {
            ...
            if (error.length() > 0) {
                pr = new ProducerRecord<String, String>(this.badTopic, "{\"error\": \" " + error + "\"}");
            } else {
                pr = new ProducerRecord<String, String>(this.goodTopic, MAPPER.writeValueAsString(root));
            }
        } catch (IOException e) {
            pr = new ProducerRecord<String, String>(this.badTopic, "{\"error\": \"" + e.getClass().getSimpleName() + ": " + e.getMessage() + "\"}");
        } finally {
            if (null != pr) {
                producer.send(pr);
            }
